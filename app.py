...
if check_password():
    ...
    # ==========================================
    # 4. ENHANCED EXPORT (Capped Period - All Cycles)
    # ==========================================
    def generate_image(ind, reg, weight, p1, p2, total, rounds, df, cap_limit):
        # 🟢 กรองตารางให้แสดงถึงรอบสุดท้ายที่อยู่ในช่วง Month <= cap_limit
        ...
        header_text = (
            ...
            f"----------------------------------------------------------------------------------------------------\n"
            f"Summary - Patient Paid Cycles: {rounds:.1f}\n"
        ...
        for (i, j), cell in the_table.get_celld().items():
            ...
            if i == 0:
                ...
                # แยกสีรอบที่ฟรี (Status: Free)
                is_free = "Free" in str(df_display.iloc[i-1]['Status'])
                if is_free:
                    cell.set_facecolor('#F9F9F9')
                    cell.set_text_props(color='#888888') # ตัวหนังสือจางลงสำหรับรอบฟรี
                else:
    ...
    # ==========================================
    # 5. INTERFACE RENDER (ORIGINAL STYLE)
    # ==========================================
    @st.cache_data
    def load_data():
    ...
    phase_html = f'<div class="card-wrapper"><div class="phase-card p1"><div class="card-label">Phase 1 / Cycle</div><div class="card-value">฿ {p1_c:,.0f}</div><div class="card-vat">● Inclusive of 7% VAT</div></div>'\
                 f'<div class="phase-card p2" style="display: {"block" if (has_p2_flag or p2_c > 0) else "none"};"><div class="card-label">Phase 2 / Cycle</div><div class="card-value">฿ {p2_c:,.0f}</div><div class="card-vat">● Inclusive of 7% VAT</div></div>'
